...
- `control-car/`：MSPM0G3507 循迹小车 + 云台 + K230 视觉训练平台
- 主控平台：TI MSPM0G3507
- 小车/云台视觉平台：立创·庐山派 K230 / CanMV K230
...
7. 处理 K230 视觉任务时必须读 `control-car/docs/k230-vision-plan.md`。
8. 查通用算法、通用硬件积木时，看 `common/`。
9. **写小车代码前**，必须读 `control-car/hardware/pinmap.md`（真实引脚映射）。
10. **调 PID 参数前**，必须读 `control-car/firmware/mspm0/config/pid-defaults.md`（起步参数）。

## 视觉平台

- **主线**：立创·庐山派 K230 / CanMV K230。
- **角色**：K230 只做视觉协处理器，不替代 MSPM0G3507 主控。
- **任务**：摄像头预览、黑线/色块识别、二维码、AprilTag、靶面定位、YOLO/kmodel 端侧推理。
- **通信**：K230 通过 UART 向 MSPM0 发送结构化视觉结果，MSPM0 做运动控制和云台控制。
- **存储**：K230 需要 TF 卡启动/保存脚本和模型，推荐 8GB/16GB Class 10 或 U1 及以上。
- **调试**：USB 摄像头 + PC OpenCV 只用于算法预研和参数调优，不作为比赛现场主线。
- **OpenMV**：不再作为小车视觉主线，只保留为旧备选/对照参考。

推荐数据流：

```text
K230 摄像头/视觉识别
  → UART: VISION,MODE=LINE,OK=1,CX=154,CY=102,ERR=-6,CONF=83
  → MSPM0G3507 解析
  → 电机 PID / 云台 PID / OLED 状态显示
```
...
D:\work\参考文件-已实现案例\DCC-100双路步进电机驱动模块资料(2)\
  ├── DCC-100v3说明书-2026-05-24.pdf
  ├── DCC-100v1v2说明书.pdf
  ├── DCC-100-stm32f103c8t6/
  └── 14_step_motor_mspm0g3507/
```
...
- K230 UART 引脚与接线原则 → `control-car/docs/k230-vision-plan.md`
- 不要使用其他来源的引脚号
...
control-car/                         小车训练平台、循迹、PID、电机、通信、显示、K230视觉
...
- K230 不参与 MSPM0 的实时电机控制，只输出视觉结果。
